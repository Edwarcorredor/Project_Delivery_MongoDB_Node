...
import { repartidorSchema, restauranteSchema, productoSchema, clienteSchema, pedidoSchema } from "../dto/modelDTO.js";
...
class ControllerCrud{
    ...
    static async setDocument(req, res){
        ...
        const result = await ModelCrud.setDocument(req.baseUrl.slice(1, -1), funMapping(validacion.data, req.baseUrl.slice(1, -1)));
    ...
    static async updateDocument(req, res){
        const validacion = schemas[req.baseUrl.slice(1, -1)].safeParse(req.body);
        if (!validacion.success) return res.status(400).json({ message: validacion.error.errors.map(error => `${error.path} - ${error.message}`)});
        const result = await ModelCrud.updateDocument(req.baseUrl.slice(1, -1), funMapping(validacion.data, req.baseUrl.slice(1, -1)), parseInt(req.params.id));
        if (result.status) return res.status(result.status).json({ message: result.message});
        res.json(result);
    }

    static async deleteDocument(req, res){
        const result = await ModelCrud.deleteDocument(req.baseUrl.slice(1, -1), parseInt(req.params.id));
        if (result.status) return res.status(result.status).json({ message: result.message});
        res.json(result);
    }
}
